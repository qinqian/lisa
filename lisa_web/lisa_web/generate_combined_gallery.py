...
result1 = list(map(lambda x: '<div class="col"><a class="" href="http://lisa.cistrome.org/new_gallery/%s_H3K27ac.html">H3K27ac</a></div>' %(x), df.iloc[:,0]))
...
def generate_page():
    ...
    with open('new_gallery.html', 'w') as outf:
        outf.write(x)
    os.system('cp new_gallery.html ../templates/new_gallery.html')
...
def generate_htmls(ids):
    ...
    for i in ids:
        ...
        if (not os.path.exists(d1)) or (not os.path.exists(d2)):
            template = env.get_template('combined_gallery_single_display.html')
            json_dict = {}
